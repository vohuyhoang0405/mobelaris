...
async function Instagram() {
  let items = Object.values(instagramEntities) as {
    photo_id: string
    products: any
  }[]
  return (
    <div className="mx-auto mb-14 mt-10 flex w-full flex-col items-center px-4 sm:px-8">
      ...
      <Container flush className="relative w-full ">
        <ul className="grid w-full list-none grid-cols-1 gap-3 gap-y-6 sm:grid-cols-2 md:grid-cols-3 md:gap-6 xl:grid-cols-4 2xlarge:grid-cols-6">
          {items.map((block, i) => {
            return (
              <li key={i} className="group relative w-full pb-[100%]">
                <Image
                  alt="Instagram"
                  disableLoader
                  width={400}
