...
async function InstagramProductPage(props: { id?: string }) {
  const id = props?.id
  let items = Object.values(instagramEntities).filter((item) => {
    return item.items.find((item) => item && item?.id === id)
  }) as {
    photo_id: string
    products: any
  }[]
  ...
  return (
    <div className="mx-auto mb-14 mt-10 flex w-full flex-col items-center px-4 sm:px-8">
      ...
      <Container flush className="relative flex w-full justify-center">
