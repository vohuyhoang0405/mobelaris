...
// import emailSubscribe from "@lib/email-subcscribe"
import { trigleGTMAddPaymentInfoEvent } from "@lib/gtm-events/add-payment-info"
import { trigleGTMenhancedConversionEvent } from "@lib/gtm-events/enhanced-conversion"
import { trigleGTMpurchaseEvent } from "@lib/gtm-events/purchase"
...
const Wrapper = dynamic(
  () => import("../templates/checkout/components/payment-wrapper")
)
...
const SHIPPING_METHOD_ON_PREFINAL_STEPS = false
...
export const CheckoutProvider = ({ children }: CheckoutProviderProps) => {
  ...
  const { resetCart, locale, region, setRegion } = useStore()
  ...
  const onPaymentCompleted = () => {
    complete(undefined, {
      onSuccess: ({ data }) => {
        setTimeout(() => {
          // TODO: Add email subscribe
          // emailSubscribe({
          //   email: data.email,
          //   from: getListId(data.shipping_address?.country_code),
          // })
          trigleGTMpurchaseEvent({ cart })
          trigleGTMAddPaymentInfoEvent({ cart })
          trigleGTMenhancedConversionEvent({ cart })
        })
        setTimeout(() => {
          router.push(`/${locale}/order/confirmed/${data.id}`)
        }, 1000)
      },
    })
  }
